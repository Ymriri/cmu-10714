...
def softmax(Z: ndl.Tensor):
    ...
    """
    # A =   # num_examples x num_classes
    Z = np.exp(Z) / np.sum(np.exp(Z), axis=1, keepdims=True)  # num_examples x num_classes
    return Z
...
def softmax_loss(Z: ndl.Tensor, y:ndl.Tensor):
    ...
    # return (ndl.log(ndl.exp(Z).sum(axes=1)) - (Z*y).sum((1,))).sum()/num_samples
    num_samples = Z.shape[0]
    return (ndl.log(ndl.exp(Z).sum((1,))) - (Z * y).sum((1,))) \
        .sum() / num_samples
