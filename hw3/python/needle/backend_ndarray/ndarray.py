...
class NDArray:
    ...
    def make(shape, strides=None, device=None, handle=None, offset=0):
        """Create a new NDArray with the given properties.  This will allocate
        the memory if handle=None, otherwise it will use the handle of an
        existing array."""
        ...
        array._handle = handle if handle is not None else array.device.Array(
            prod(shape))
        return array
    ...
    def as_strided(self, shape, strides):
        ...
        # TODO(zhangfan): why offset is not copied?
    ...
    def reshape(self, new_shape):
        ...
        if prod(self.shape) != prod(new_shape):
            raise ValueError(f"{self.shape} can't be reshaped into {new_shape}")
        if not self.is_compact():
            raise ValueError(
                f"matrix is not compact. shape is {self.shape}; strides are {self.strides}")
    ...
    def permute(self, new_axes):
        ...
        Returns:
            ...
            strides changed).
        """
        new_shape = permute(self.shape, new_axes)
        new_strides = permute(self.strides, new_axes)
        return self.as_strided(new_shape, new_strides)
    ...
    def broadcast_to(self, new_shape):
        ...
        assert all(d1 == 1 or d1 == d2 for d1, d2 in zip(self.shape, new_shape))
    ...
    def __getitem__(self, idxs):
        ...
        if not isinstance(idxs, tuple):
            idxs = (idxs,)
        idxs = tuple([
            self.process_slice(s, i)
            if isinstance(s, slice) else slice(s, s + 1, 1)
        ...
        new_strides = []
        new_shape = []
        for slice_, stride in zip(idxs, self.strides):
            assert isinstance(slice_, slice)
            new_offset += slice_.start * stride
            new_strides.append(slice_.step * stride)
            new_shape.append(
                (slice_.stop - slice_.start - 1) // slice_.step + 1)

        return self.make(tuple(new_shape), strides=tuple(new_strides),
                         device=self.device,
    ...
    def ewise_or_scalar(self, other, ewise_func, scalar_func):
        ...
        if isinstance(other, NDArray):
            ...
            ewise_func(self.compact()._handle, other.compact()._handle,
                       out._handle)
        else:
    ...
    def __matmul__(self, other):
        ...
        if hasattr(self.device, "matmul_tiled") and all(
            ...
            self.device.matmul(
                self.compact()._handle, other.compact()._handle, out._handle, m,
                n, p
            )
            return out
