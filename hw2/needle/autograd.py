...
class Tensor(Value):
    ...
    def debug_shape(self) -> str:
        return f"{self.op.__class__.__name__} {self.data.shape}"
...
def compute_gradient_of_variables(output_tensor: Tensor, out_grad: Tensor):
    ...
    for node in reverse_topo_order:
        ...
        partial_grads = node.op.gradient(grad, node)
        if isinstance(partial_grads, Value):
            partial_grads = [partial_grads]
        assert len(node.inputs) == len(partial_grads)
